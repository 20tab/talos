...
from digitalocean.baseapi import BaseAPI, Error, GET, POST, DELETE
...
class Cluster(BaseAPI):
    ...
    Args:
        ...
        region (str): The slug identifier for the region where the Kubernetes cluster will be created.
        version (str): The slug identifier for the version of Kubernetes used for the cluster. See the /v2/kubernetes/options endpoint for available versions.
        ...
        tags (list): A flat array of tag names as strings to be applied to the Kubernetes cluster. All clusters will be automatically tagged "k8s" and "k8s:$K8S_CLUSTER_ID" in addition to any tags provided by the user. 
        maintenance_policy (object): An object specifying the maintenance window policy for the Kubernetes cluster (see table below). 
    ...
    Attributes returned by API:
        ...
        tags (list):s An array of tags applied to the Kubernetes cluster. All clusters are automatically tagged "k8s" and "k8s:$K8S_CLUSTER_ID."
        maintenance_policy (object): An object specifying the maintenance window policy for the Kubernetes cluster (see table below).
        node_pools (list): An object specifying the details of the worker nodes available to the Kubernetes cluster (see table below).
        ...
        status (object): An object containing a "state" attribute whose value is set to a string indicating the current status of the node. Potential values include running, provisioning, and errored.
